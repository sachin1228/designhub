...
// All avatar source values the UI can send.
// "upload" is handled by the multipart path and never reaches
// the URL-selection validation, but is included for completeness.
...
export async function POST(request: NextRequest) {
  ...
  if (contentType.includes("multipart/form-data")) {
    ...
    if (uploadError) {
      console.error(
        "[signup/avatar] storage upload error:",
        uploadError
      );
    ...
    if (dbError) {
      console.error(
        "[signup/avatar] profile update error:",
        dbError
      );
    ...
    // All 3 signup steps are complete.
    // Mark invitation as used and send welcome email.
  ...
  // DiceBear / Boring Avatars / Robohash / Avataaars / Multiavatar
  ...
  if (
    !avatar_source ||
    !ALLOWED_SOURCES.includes(avatar_source as never)
  ) {
    return NextResponse.json(
      { error: "Invalid avatar_source." },
      { status: 422 }
    );
  }
