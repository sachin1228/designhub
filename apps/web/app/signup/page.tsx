...
// Note: avataaars / avataaars-neutral have their own dedicated tab below.
...
// Avataaars — the original Avataaars design library (Fang-Pen Lin), hosted
// on the DiceBear CDN. Gets its own tab so users can browse both styles
// (with/without background) without hunting through the full DiceBear list.
const ALL_AVATAAARS_STYLES: { style: string; label: string }[] = [
  { style: "avataaars",         label: "Avataaars" },
  { style: "avataaars-neutral", label: "Avataaars Neutral" },
];

// Multiavatar — 12 billion+ unique SVGs via free CDN, no API key required.
// Seeds are strings; any change produces a visually distinct avatar.
// We generate 24 seeds per user (name + index suffixes) to show good variety.
const MULTIAVATAR_SEED_COUNT = 24;
...
function getAllAvatarOptions(name: string): AvatarOption[] {
  ...
  const seedNames6 = Array.from({ length: 6 }, (_, i) =>
    i === 0 ? baseName : `${baseName} ${i + 1}`
  );
  // Avataaars: 12 seeds per style → more visible variety per tab
  const seedNames12 = Array.from({ length: 12 }, (_, i) =>
    i === 0 ? baseName : `${baseName} ${i + 1}`
  );
  // Multiavatar: 24 seeds to showcase the library's huge range
  const seedNamesMulti = Array.from({ length: MULTIAVATAR_SEED_COUNT }, (_, i) =>
    i === 0 ? baseName : `${baseName} ${i + 1}`
  );

  const dicebear = ALL_DICEBEAR_STYLES.flatMap(({ style, label }) =>
    seedNames6.map((seedName, index) => {
  ...
  // Avataaars — dedicated tab: both regular and neutral variants, 12 seeds each
  const avataaars = ALL_AVATAAARS_STYLES.flatMap(({ style, label }) =>
    seedNames12.map((seedName, index) => {
      const seed = encodeURIComponent(seedName);
      return {
        id: `avataaars-${style}-${index}`,
  ...
  // Multiavatar — 24 seeds, each a fully unique SVG illustration
  const multiavatar = seedNamesMulti.map((seedName, index) => ({
    id: `multiavatar-${index}`,
    source: "multiavatar" as AvatarSource,
    style: "multiavatar",
    label: index === 0 ? "Multiavatar" : `Multiavatar ${index + 1}`,
...
function SignupInner() {
  ...
  const visibleAvatarOptions =
    activeAvatarTab === "avataaars"
      ? avatarSourceOptions.avataaars
      : activeAvatarTab === "multiavatar"
      ? avatarSourceOptions.multiavatar
      : avatarSourceOptions[activeAvatarTab as "dicebear" | "boring-avatars" | "robohash"];
          ...
          {tokenState.status === "valid" && step === 3 && (
            <div className="rounded-xl border border-overlay-elevated bg-overlay-raised p-8 shadow-xl">
              ...
              <div className="mb-5 overflow-hidden rounded-xl border border-overlay-elevated">
                {/* Tab bar — horizontally scrollable so all 5 tabs fit on narrow screens */}
                <div className="flex gap-1 overflow-x-auto border-b border-overlay-elevated bg-overlay px-2 py-2 scrollbar-none">
                  {avatarTabs.map((tab) => (
                    <button
                      key={tab.key}
                      type="button"
                      onClick={() => setActiveAvatarTab(tab.key)}
                      className={`flex shrink-0 items-center gap-1 rounded-md px-3 py-2 font-body text-xs font-medium transition-colors ${
